...
import { Activity, CheckCircle2, LayoutGrid, KeyRound, Map, ScrollText, Settings, X, } from "lucide-react";
import { cn } from "../utils/cn";
import { Eyebrow } from "./ui";
import { SCREEN_LABEL } from "../api/sitesecurityTypes";
const NAV = [
...
export default function Sidebar({ screen, onNavigate, }) {
    ...
    return (<>
      <aside className="fixed inset-y-0 left-0 z-40 flex w-[240px] flex-col bg-[#0a8276] text-white shadow-lg">
        ...
        <div className="border-b border-white/15 px-4 pb-5 pt-6 text-center">
          <div className="mx-auto flex size-24 items-center justify-center overflow-hidden rounded-full bg-white p-1 shadow-md">
            <img src="/logo.png" alt="Logo" className="h-full w-full object-contain scale-[1.55]"/>
          </div>
        ...
        <nav className="flex-1 space-y-1.5 px-3 pt-5">
          ...
          {NAV.map((item) => {
            ...
            return (<button key={item.id} onClick={() => onNavigate(item.id)} className={cn("group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] font-normal transition-all duration-200", active
                    ? "bg-white/20 text-white font-medium shadow-sm backdrop-blur-sm"
                    : "text-white/80 hover:bg-white/10 hover:text-white")}>
                <Icon className={cn("size-[18px]", active ? "text-white" : "text-white/70 group-hover:text-white")} strokeWidth={2}/>
                <span className="font-sans">{SCREEN_LABEL[item.id]}</span>
                {item.id === "log" && (<span className={cn("ml-auto rounded-full px-2 py-0.5 font-mono text-[10px] font-semibold", active ? "bg-[#ef6c00] text-white" : "bg-white/20 text-white")}>
                    1
                  </span>)}
                {active && item.id !== "log" && (<span className="ml-auto size-1.5 rounded-full bg-[#ef6c00]"/>)}
              </button>);
        })}
        </nav>
        ...
        <div className="px-3 pb-3 pt-3">
          ...
          <div className="relative">
            <button onClick={() => setShowHealth((v) => !v)} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] font-normal transition-colors", showHealth ? "bg-white/20 text-white" : "text-white/80 hover:bg-white/10 hover:text-white")}>
            ...
            <AnimatePresence>
              {showHealth && (<motion.div initial={{ opacity: 0, y: 10, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.97 }} transition={{ duration: 0.18 }} className="absolute bottom-full left-0 z-50 mb-2 w-[220px] rounded-2xl border border-slate-200 bg-white p-4 text-navy-900 shadow-[0_24px_60px_-20px_rgba(10,17,31,0.35)]">
                  <Eyebrow className="mb-3 text-slate-400">Site health — simulated</Eyebrow>
                  <div className="space-y-2.5">
                    {HEALTH.map((row) => (<div key={row.label} className="flex items-center justify-between text-[12px]">
          ...
          <button onClick={() => setShowSettings(true)} className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[14px] font-normal text-white/80 transition-colors hover:bg-white/10 hover:text-white">
      ...
      <AnimatePresence>
        {showSettings && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-navy-950/40 p-6 backdrop-blur-sm" onClick={() => setShowSettings(false)}>
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 12 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 12 }} transition={{ duration: 0.2 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-[420px] rounded-2xl border border-slate-200 bg-white p-6 shadow-[0_24px_60px_-20px_rgba(10,17,31,0.35)]">
              <div className="flex items-start justify-between">
                ...
                <button onClick={() => setShowSettings(false)} className="rounded-lg p-1.5 text-slate-400 transition-colors hover:bg-slate-100 hover:text-navy-900">
                  <X className="size-4"/>
                </button>
              </div>
              <div className="mt-5 space-y-4">
                <label className="flex cursor-pointer items-center justify-between text-[13px] font-medium text-navy-900">
                  ...
                  <button role="switch" aria-checked={hints} onClick={(e) => {
                e.preventDefault();
                setHints((v) => !v);
            }} className={cn("relative h-6 w-11 rounded-full transition-colors", hints ? "bg-navy-900" : "bg-slate-200")}>
                    <span className={cn("absolute top-0.5 size-5 rounded-full bg-white shadow transition-all", hints ? "left-[22px]" : "left-0.5")}/>
                  </button>
                </label>
