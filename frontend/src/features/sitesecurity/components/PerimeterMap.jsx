...
/* ------------------------------------------------------------------ */
const BOUNDS = "M 96,320 C 88,206 150,118 262,96 C 372,74 540,88 620,158 C 678,214 686,344 626,422 C 570,496 452,532 330,514 C 214,497 104,434 96,320 Z";
const ZONE_RECTS = {
...
const GATES = [
    { id: "main", name: "Main Entrance", icon: DoorOpen, x: 465, y: 510, dot: "#10B981", lx: 465, ly: 554, w: 128, anchor: "middle" },
    { id: "visitor", name: "Visitor Gate", icon: UserRound, x: 96, y: 320, dot: "#10B981", lx: 162, ly: 320, w: 100, anchor: "middle" },
    { id: "staff", name: "Staff / Operational Gate", icon: HardHat, x: 667, y: 282, dot: "#10B981", lx: 587, ly: 236, w: 158, anchor: "middle" },
    { id: "service", name: "Service / Emergency Gate", icon: Truck, x: 452, y: 92, dot: "#D97706", lx: 452, ly: 142, w: 168, anchor: "middle" },
];
function LabelChip({ x, y, w, color, text, size = 9.5, }) {
    return (<g className="pointer-events-none select-none">
...
export default function PerimeterMap({ selected, onSelect, interactive = true, compact = false, }) {
    ...
    return (<div className="relative">
      <svg viewBox="0 0 760 600" className="h-auto w-full" role="img" aria-label="Conceptual perimeter diagram">
        ...
        <g {...zoneProps("open")}>
          ...
          <path d={BOUNDS} fill="none" stroke="#101B2E" strokeWidth={selected === "open" || hover === "open" ? 6.5 : 5} strokeLinecap="round" strokeDasharray="0.1 21"/>
        ...
        {["controlled", "restricted", "critical"].map((id) => {
            const z = ZONE_RECTS[id];
            const meta = LEVEL_META[id];
            return (<g key={id} {...zoneProps(id)}>
              {selected === id && (<rect x={z.x - 5} y={z.y - 5} width={z.w + 10} height={z.h + 10} rx={z.rx + 5} fill="none" stroke={meta.solid} strokeOpacity={0.25} strokeWidth={7}/>)}
              <rect x={z.x} y={z.y} width={z.w} height={z.h} rx={z.rx} fill={meta.fill} stroke={meta.solid} strokeOpacity={hover === id || selected === id ? 0.85 : 0.45} strokeWidth={strokeFor(id)}/>
              {id !== "controlled" && (<rect x={z.x} y={z.y} width={z.w} height={z.h} rx={z.rx} fill={`url(#hatch-${id === "restricted" ? "amber" : "rose"})`} className="pointer-events-none"/>)}
            </g>);
        })}

        {/* zone labels */}
        <LabelChip x={150} y={172} w={96} color={LEVEL_META.open.solid} text="OPEN ZONE"/>
        <LabelChip x={ZONE_RECTS.controlled.labelX} y={ZONE_RECTS.controlled.labelY} w={ZONE_RECTS.controlled.labelW} color={LEVEL_META.controlled.solid} text="CONTROLLED ZONE"/>
        <LabelChip x={ZONE_RECTS.restricted.labelX} y={ZONE_RECTS.restricted.labelY} w={ZONE_RECTS.restricted.labelW} color={LEVEL_META.restricted.solid} text="RESTRICTED ZONE"/>
        <LabelChip x={ZONE_RECTS.critical.labelX} y={ZONE_RECTS.critical.labelY} w={ZONE_RECTS.critical.labelW} color={LEVEL_META.critical.solid} text="CRITICAL"/>

        {/* gates */}
        {GATES.map((g) => {
            const Icon = g.icon;
            return (<g key={g.id} className="cursor-default">
              <title>{g.name}</title>
              {g.id === "service" && (<circle cx={g.x + 13} cy={g.y - 13} r="8" fill="#D97706" opacity={0.5} className="gate-ping"/>)}
              <rect x={g.x - 16} y={g.y - 16} width={32} height={32} rx={9} fill="white" stroke="#101B2E" strokeWidth={1.4}/>
              ...
              <title>{g.name}</title>
              {!compact && (<LabelChip x={g.lx} y={g.ly} w={g.w} color="#475569" text={g.name.toUpperCase()} size={8}/>)}
            </g>);
        })}
