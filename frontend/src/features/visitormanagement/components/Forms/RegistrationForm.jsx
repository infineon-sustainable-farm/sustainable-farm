 ...
 * directory first — their details are fetched and reused, no re-typing. If the
 * typed name is not in the directory, an error is shown and nothing is
 * created; the visitor must be placed in the directory first. Then the target
 * is chosen: a farm tour slot, an event, or an activity booking. In edit mode
 * it only edits the visitor fields; the API has no endpoint to move a
 * registration to another slot or event.
 */
export default function RegistrationForm({
     ...
     * their fields are prefilled from the record and the registration is
     * created against that visitor instead of a new one. A name that does not
     * match anything is an error — no visitor is created.
     */
    function handleNameChange(value) {
        ...
        if (match) {
            setGroupSize(String(match.groupSize ?? 1));
            setEmail(match.email ?? "");
            setPhone(match.phone ?? "");
            setLanguage(match.language ?? "French");
            setVisitorType(
                match.type === "SCHOOL"
                    ? "SCHOOL"
                    : match.type === "PARTNER"
            ...
            setSpecialNeeds(match.specialNeeds ?? "");
        }
        setFieldErrors((errors) => {
            const rest = { ...errors };
            delete rest.fullName;
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input
                        ...
                        placeholder="Type a full name — found in the directory"
                        className={INPUT_CLASS}
                    />
                    {!isEditing && fetchedVisitor && (
                        <p className="mt-1 text-[11px] text-muted">
                            Found in the directory: {fetchedVisitor.fullName} ·{" "}
                            {formatEnumLabel(fetchedVisitor.type)} · {fetchedVisitor.groupSize}{" "}
                    ...
                    {!isEditing &&
                        ...
                        !fetchedVisitor && (
                            <p className={ERROR_CLASS}>
                                No visitor with this name in the directory — place the visitor in
                                the Visitors screen first.
                            </p>
                        )}
                    {fieldError("fullName") && (
